...
const chat_openers_prompt = "Summarization of chat first sentence for menu items:\n\n"
...
export async function* generateAnswer(messages, activePrompt, model) {
    ...
    while (isUnfinished && (tries < model.maxTries)) {
        ...
        const lastResult = await complete(
            prompt + compoundedResult, model, stop_sequences, true);
...
export async function getChatName(prompt, model) {
    const summary_prompt = chat_openers_prompt
    + `### Input: """${prompt}"""\n`
    + "### Summary:\n";

    const result = (await complete(
        summary_prompt, model, ["\n","<|endoftext|>"], false)).replace("#", "").trim();
